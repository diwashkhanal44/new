...
enum NewsType { Category, Topic }

class CategoryWiseNewsRepository {
  final HttpCalls _httpCalls;

  CategoryWiseNewsRepository({required HttpCalls httpCalls})
      : _httpCalls = httpCalls;

  Future<Either<Failure, NewsModel>> getCategoryWiseNews({
    int? page,
    int? rowsPerPage,
    ...
    try {
      final uri = Uri(
          ...
          port: Endpoints.port,
          path: newsType == NewsType.Category
              ? Endpoints.categoriyWiseNews
                  .replaceAll('{category-slug}', categorySlug)
              : Endpoints.newsByTopic.replaceAll('{topicslug}',
                  categorySlug), // todo: change this to match category
          queryParameters: {
